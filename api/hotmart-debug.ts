...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const vendas = rows.slice(1).map((row, idx) => {
      ...
      return {
        ...
        preco,
        temMentoria: nomeProduto.includes('Mentoria Start 90'),
      };
    });
    ...
    res.status(200).json({
      ...
      totalMentoria: vendas.filter(v => v.temMentoria).length,
      filteredMentoria: vendasMentoria.length,
      startDateObj: startDateObj?.toISOString(),
      endDateObj: endDateObj?.toISOString(),
      amostra: vendas.filter(v => v.temMentoria).slice(0, 5),
      filteredAmostra: vendasMentoria.slice(0, 5),
    });
  } catch (error) {
    res.status(500).json({
